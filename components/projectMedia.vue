  ...
  <div class="media" v-if="media">
    ...
    <div v-else-if="type === 'youtube'">
      <iframe
          ...
          <a class='ytImgLink' href='https://www.youtube.com/embed/${yt_id}'>
            <img src='https://img.youtube.com/vi/${yt_id}/hqdefault.jpg' alt='Video Photo'>
    ...
    <div v-else-if="type === 'github'">
      <a :href="href" target="_blank" class="githubContainer">
        <div class="github"></div>
        <p class="name">{{ name }}</p>
      </a>
    </div>

    <div v-else-if="type === 'fbgroup'">
      <a :href="href" target="_blank" class="fbgroup">
        <img :src="img" :alt="name + ' facebook group'" />
      </a>
    </div>

    <div v-else-if="type === 'fbpost'">
      <div class="fbpost">
        <iframe 
          class='fbpost'
          width="500"
      ...
      if(typeof this.media === 'string'){
        ...
        for( const parts of this.media.split(".") ){
          if(parts == 'youtube'){
            ...
            // this.yt_url = `https://www.youtube.com/watch?v=${this.yt_id}`
            this.yt_url = `https://www.youtube.com/v/${this.yt_id}`
            // https://img.youtube.com/vi/X7MoF4tvf2Q/hqdefault.jpg
            // this.yt_img = this.yt_url.replace("www","img").replace("embed","vi") + "/hqdefault.jpg";
            this.yt_img = `https://img.youtube.com/vi/${this.yt_id}/hqdefault.jpg`;
            return this.type = "youtube"
          }
        }
      }
      if(typeof this.media === 'object'){
        if(this.media.type){ this.type = this.media.type }
        if(this.media.href){ this.href = this.media.href }
        if(this.media.img){ this.img = this.media.img }
        if(this.media.id){ this.id = this.media.id }
        this.name = this?.href?.split("/")[ this?.href?.split("/")?.length - 1 ]
      }
      return this.media
    },
  },
